...
template <typename T>
void HttpSimpleControllersRouter::doAdvicesChain(const std::vector<T> &advices,
                                                 size_t index,
                                                 const HttpRequestImplPtr &req,
                                                 ...
                                                 std::function<void()> finalCallback)
{
    ...
    advices[index](req, [this, &advices, index, req, callbackPtr, finalCallback](const HttpResponsePtr &resp) {
        if (resp)
        {
            (*callbackPtr)(resp);
        }
        ...
        {
            doAdvicesChain(advices, index + 1, req, callbackPtr, finalCallback);
        }
    });
}