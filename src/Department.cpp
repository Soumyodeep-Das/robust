...
const std::vector<Department::MetaData> Department::metaData_ = {
    {"id", "int32_t", "integer", 4, true, false, true},
    {"name", "std::string", "character varying", 50, false, false, false}
};
...
const Department::PrimaryKeyType &Department::getPrimaryKey() const
{
    assert(id_);
    return *id_;
}
...
Json::Value Department::toMasqueradedJson(const std::vector<std::string> &pMasqueradingVector) const
{
    Json::Value ret;
    if(pMasqueradingVector.size() == 2)
    {
...
{
    static const std::string sql = "select * from person where department_id = $1";
    *clientPtr << sql
               << *id_
