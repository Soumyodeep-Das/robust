...
bool HttpClientImpl::onDefaultPort() const
{
    if (useSSL_)
    {
        return serverAddr_.port() == 443;
    }
    else
    {
        return serverAddr_.port() == 80;
    }
}

std::string HttpClientImpl::host() const
{
    return domain_;
}

uint16_t HttpClientImpl::port() const
{
    return serverAddr_.port();
}