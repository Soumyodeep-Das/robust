...
const std::vector<User::MetaData> User::metaData_ = {
    {"id", "int32_t", "integer", 4, true, false, true},
    {"username", "std::string", "character varying", 50, false, false, false},
    {"password", "std::string", "character varying", 0, false, false, false}
};
...
const User::PrimaryKeyType &User::getPrimaryKey() const
{
    assert(id_);
    return *id_;
}
...
Json::Value User::toMasqueradedJson(const std::vector<std::string> &pMasqueradingVector) const
{
    Json::Value ret;
    if (pMasqueradingVector.size() == 3)
    {
